...
/*
 * MegaMUD's Wait For Party Members (2026-09-24): leading, the walk stands still
 * while a member's listed health is under the line, whether or not they said
 * @wait, and If Leading Wait No Longer Than bounds it.
 */
describe('leading a party', () => {
  ...
  const leading = (health: number | null, following: string | null = null): CharacterState => {
    const state = at(1, 1);
    state.name = 'Skinny';
    state.vitals = { ...state.vitals, hp: 100, hpMax: 100 };
    state.party = {
      ...state.party,
      following,
      members: [member('Skinny', 1), member('Healbot', health), member('Jack', 0.1, true)]
    };
    return state;
  };
  const walkerFor = (
    party: Partial<AutomationConfig['party']>
  ): { walk: Walker; heal: (health: number | null) => void } => {
    let current = leading(0.3);
    const walk = new Walker(
      {
        ...config,
        health: { ...config.health, restBelow: 0 },
    ...
    return {
      walk,
      heal: (health) => {
        current = leading(health);
      }
    };
  };

  it('stands still while a member is under the line, and walks on when they are not', async () => {
    const { walk, heal } = walkerFor({ waitForMembersBelow: 0.5 });
    expect(walk.start(ROUTE, leading(0.3))).toBeNull();
    await vi.advanceTimersByTimeAsync(50);
    expect(sent).toEqual([]);
    expect(walk.progress.hold).toBe('party');
    expect(notices.some((n) => /Healbot is at 30%/.test(n))).toBe(true);
    heal(0.8);
    await vi.advanceTimersByTimeAsync(2_000);
    expect(sent).toEqual(['e']);
    walk.dispose();
  ...
  /* An invitation nobody accepted is not a member, and following, the leader decides. */
  it('reads neither an invitation nor the party it follows', async () => {
    const { walk } = walkerFor({ waitForMembersBelow: 0.5 });
    expect(walk.start(ROUTE, leading(0.9))).toBeNull();
    await vi.advanceTimersByTimeAsync(50);
    expect(sent).toEqual(['e']);
    walk.dispose();

    sent = [];
    const follower = new Walker(
      { ...config, party: { ...config.party, waitForMembersBelow: 0.5 } },
      queue,
      { stateNow: () => leading(0.3, 'Healbot') }
    );
    expect(follower.start(ROUTE, leading(0.3, 'Healbot'))).toBeNull();
    await vi.advanceTimersByTimeAsync(50);
    expect(sent).toEqual(['e']);
    follower.dispose();
  });

  it('walks on once it has waited as long as it was told to', async () => {
    const { walk } = walkerFor({ waitForMembersBelow: 0.5, waitNoLongerMinutes: 1 });
    expect(walk.start(ROUTE, leading(0.3))).toBeNull();
    await vi.advanceTimersByTimeAsync(30_000);
    expect(sent).toEqual([]);
    await vi.advanceTimersByTimeAsync(40_000);
    // The step, and whatever the walk does about an answer the fixture never gives.
    expect(sent[0]).toBe('e');
    expect(notices.some((n) => /Waited 1 min for Healbot/.test(n))).toBe(true);
    walk.dispose();
  });

  it('does not wait at all when the line is off', async () => {
    const { walk } = walkerFor({ waitForMembersBelow: 0 });
    expect(walk.start(ROUTE, leading(0.1))).toBeNull();
    await vi.advanceTimersByTimeAsync(50);
    expect(sent).toEqual(['e']);
    walk.dispose();
  });
